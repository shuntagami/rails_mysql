...
WORKDIR /opt/app
RUN <<eot
apt-get update -qq && apt-get install -y build-essential default-libmysqlclient-dev apt-transport-https curl git wget
...
ENV RACK_ENV=production
EXPOSE 3000
RUN apt-get update -qq && apt-get install -y build-essential default-libmysqlclient-dev apt-transport-https
WORKDIR /opt/app
